...
def read_ply_ascii_geo(filedir,encode=True):
    if encode:
        pcd=open3d.t.io.read_point_cloud(filedir)
        coords = pcd.point.positions[:,0:3].numpy().astype('int')
    else:
        files = open(filedir)
        data = []
...
def isin(data, ground_truth):
    ...
    combined=torch.cat([data,ground_truth])
    combined_val=combined.unique(return_counts=True,return_inverse=True,sorted=False)
    idx=(combined_val[1])[0:len(data)]
    # aa=(combined_val[0])[idx]
    count=(combined_val[2])[idx]
    # intersection=combined_val[counts>1]
    # torch.where()
    mask=count>1
    return mask
# def isin(data, ground_truth):
...
def sort_spare_tensor(sparse_tensor):
    ...
    """
    indices_sort = torch.argsort(array2vector_torch(sparse_tensor.C,
