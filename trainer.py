...
class Trainer():
    def __init__(self, config, model):
        ...
        self.lr_schedule = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=10, gamma=0.5)
        ...
        self.epoch = 134
        self.record_set = {'bce':[],'mse':[], 'bces':[], 'bpp_A':[], 'bpp_G':[], 'bpp':[],'sum_loss':[]}
        self.loss=1e4
    ...
    def load_state_dict(self):
        ...
        if self.config.init_ckpt=='':
            ...
            #
            #
            # ckpt_G = torch.load((dir + '/epoch_285.pth'), map_location=torch.device('cpu'))
            # ckpt_G = ckpt_G['model']
            ...
            #     if name.startswith("bitEstimator_z_G."):
            #         new_state_dict[name] = param
            #     if name.startswith("hyer_encoder_G."):
            #         new_state_dict[name] = param
            #     if name.startswith("hyer_decoder_G."):
            #         new_state_dict[name] = param
            ...
            new_state_dict_load=torch.load((dir + '/epoch_133.pth'), map_location=torch.device('cpu'))['model']
    ...
    def test(self, dataloader, main_tag='Test'):
        ...
        for batch_step, (coords, feats, color) in enumerate(tqdm(dataloader)):
            ...
            sum_loss = self.config.alpha * (bce )+ self.beta * bpp+self.config.lamda *mse_c            # for out_cls, ground_truth in zip(out_set['out_cls_list'], out_set['ground_truth_list']):
    ...
    def train(self, dataloader):
        ...
        for batch_step, (coords, feats,color) in enumerate(tqdm(dataloader)):
            ...
            sum_loss = self.config.alpha * (bce )+ self.beta * bpp+self.config.lamda *mse_c            # backward & optimize
            ...
            with torch.no_grad():
                if self.config.rank == 0:
                    mean_loss.append(sum_loss.item())
        ...
        with torch.no_grad():
            if self.config.rank==0:
                ...
                self.logger.info('one epoch point mean loss: %f' % ml)
